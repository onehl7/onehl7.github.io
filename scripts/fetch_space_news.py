...
print("\nFetching space news...")
for source, data in space_feeds.items():
    url = data["url"]
    count = data["count"]
    print(f"- {source}: {url}")
...
all_space_news.sort(key=lambda x: x["published"], reverse=True)

data_dir = os.path.join(os.path.dirname(__file__), '..', '_data')
if not os.path.exists(data_dir):
    os.makedirs(data_dir)

with open(os.path.join(data_dir, 'space_news.json'), 'w') as f:
    json.dump(all_space_news, f, indent=2)

print("Successfully saved space news data.")